...
export function App() {
  ...
  return (
    <>
      ...
      {search && <ProductSearchDialog {...search} onClose={() => setSearch(null)} onSelected={(name) => { setAnnouncement(`${name}${search.target.mode === 'add' ? 'を構成に追加しました' : 'に置き換えました'}`); setSearch(null) }} />}
    </>
  )
}
